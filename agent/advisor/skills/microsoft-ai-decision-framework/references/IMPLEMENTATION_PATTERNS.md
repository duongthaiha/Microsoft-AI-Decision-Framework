...
- Service-account execution must be reviewed to avoid over-privileged actions (align with the [Action Safety Guardrail Playbook](EVALUATION_CRITERIA.md#action-safety-guardrail-playbook)).
...
- [Foundry Agent Service overview](https://learn.microsoft.com/en-us/azure/ai-foundry/agents/overview?view=foundry)[^agentservice-overview]
- [Microsoft AI Feature Comparison](FEATURE_COMPARISON.md)[^feature-comparison]
- [Evaluation Criteria – Governance & Compliance](EVALUATION_CRITERIA.md#governance--compliance)[^evaluation-governance]
...
1. **Run the BXT gate.** Confirm viability, desirability, and feasibility in [Decision Framework Phase 1](DECISION_FRAMEWORK.md#phase-1-business-impact-assessment-bxt-framework). Any red score means pause here. Capture the scenario in the [Scenarios backlog](SCENARIOS.md#how-to-use-this-guide)) instead of forcing a pattern.
...
3. **Clarify delivery ownership (avoid the low-code vs pro-code trap).** Use the skills, time, and funding matrices in [Evaluation Criteria](EVALUATION_CRITERIA.md#skills--resources). Decide who will build and run the backlog, not which UI they click.  
...
4. **Decide the governance boundary.** Align with the governance table in [Evaluation Criteria](EVALUATION_CRITERIA.md#governance--compliance).  
...
5. **Assess orchestration complexity.** Use the orchestration complexity matrix in [Quick Reference](QUICK_REFERENCE.md#orchestration-complexity-decision-matrix).  
...
**Before you decide:** Validate your choice against the scenario playbooks in [Scenarios](SCENARIOS.md) and the capability groupings in [Capability Model](CAPABILITY_MODEL.md) to ensure the pattern supports the right part of Microsoft’s AI portfolio. Use the comparison matrices in [Quick Reference](QUICK_REFERENCE.md) to double-check complexity, skills, budget, and governance trade-offs.
...
> Related Resources
>
> - For technology comparison tables, see [Quick Reference](QUICK_REFERENCE.md)
> - For real-world applications of these patterns, see [Scenarios](SCENARIOS.md)
> - For evaluation criteria, see [Evaluation Criteria](EVALUATION_CRITERIA.md)
> - For guardrail guidance, see [Action Safety Guardrail Playbook](EVALUATION_CRITERIA.md#action-safety-guardrail-playbook)
