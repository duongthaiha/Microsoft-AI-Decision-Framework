...
#### dallas-sessions-500-diagnostics

# Decision: /sessions 500 — ManagedIdentityCredential clientId Missing

**Author:** Dallas  
**Date:** 2026-05-27  
**Status:** Fixed and deployed (revision `advisor-agent-app--azd-1779873274`)

---

## Symptom

After the 401 auth cascade was resolved, the SPA showed:

> Could not load sessions (API GET /sessions failed: 500). The backend may not be deployed yet.

## Root Cause

The Container App uses a **user-assigned** managed identity (`advisor-agent-identity`, clientId `141376cf-4b1e-4069-8aa5-5a7079c29141`). Both `cosmos-client.ts` and `identity.ts` called `new ManagedIdentityCredential()` **without specifying the `clientId`**.

`ManagedIdentityCredential` with no arguments targets the system-assigned identity. The Container App has no system-assigned identity — only user-assigned — so every Cosmos DB call failed immediately with:

```
CredentialUnavailableError: ManagedIdentityCredential: Authentication failed. Message undefined:
  Error(s): Not Available - Description: Unable to load the proper Managed Identity.
```

This threw synchronously inside `listSessions()` → `handleError()` → `res.status(500).json(...)`.

**Evidence (ACA logs, revision `advisor-agent-app--azd-1779868342`):**
```
2026-05-27T09:08:59Z [responses-adapter] error: CredentialUnavailableError: 
  ManagedIdentityCredential: Authentication failed. Message undefined: 
  Error(s): Not Available - Description: Unable to load the proper Managed Identity. 
  - Correlation ID: 786b0952-f813-402c-9045-41fc79a8605a
```

The error surfaced on the **first** Cosmos call (org-context seed check on boot), confirming the credential selection was wrong from the start.

**Cosmos DB RBAC was correct:** Principal `c8c13fe3-325a-439b-8aa8-d365f3ebe285` (the managed identity's principalId) has `Cosmos DB Built-in Data Contributor` at account scope — not a permissions issue.

## Fix

Surgical two-file change: pass `AZURE_CLIENT_ID` (already injected by Bicep into the container env) to `ManagedIdentityCredential` when available.

**`agent/src/data/cosmos-client.ts`:**
```typescript
// Before:
...
**`agent/src/auth/identity.ts`** — same pattern for the AOAI/AI Search credential.

Reference: [ManagedIdentityCredential with user-assigned identity](https://learn.microsoft.com/azure/container-apps/managed-identity) — when using user-assigned identity, you must pass the `clientId` (or `resourceId`) to select it.

## Verification

- `npm test` in `agent/`: **34/34 pass** (unchanged)
- `azd deploy agent`: deployed as revision `advisor-agent-app--azd-1779873274`
- Post-deploy ACA logs: **zero** `CredentialUnavailableError` entries; clean startup confirmed

## Open Follow-ups

1. **Parker:** The invariant should live in `infra/modules/container-apps.bicep` as a comment — `AZURE_CLIENT_ID` MUST match the user-assigned identity's `clientId`. This is already the case but worth a doc note.
2. **Future deployments:** If the identity is ever replaced, ensure the new clientId is propagated to `AZURE_CLIENT_ID` env var before deploying new code.

---

#### dallas-sessions-502-diagnostics

# Decision: Sessions 502 — Cosmos DB Public Network Access Disabled

**Author:** Dallas  
**Date:** 2026-05-27T09:21:29Z  
**Revision fixed:** `advisor-agent-app--azd-1779874793`  
**Supersedes:** dallas-sessions-500-diagnostics.md (previous fix)

---

## Symptom

`GET /sessions` returned 502. `/health` returned 200. The ACA revision `advisor-agent-app--azd-1779873274` was healthy (1/1 replica, `provisioningState: Provisioned`) and the previous `AZURE_CLIENT_ID` fix was confirmed deployed. 502 appeared only for endpoints that touch Cosmos DB.

---

## Root Cause

**Primary:** Cosmos DB account `advisor-cosmos-uwmrjzgkhs2hk` had `publicNetworkAccess: Disabled`. The Container App's outbound IP (`135.116.244.51`) was blocked by Cosmos DB's network-level firewall before any RBAC check. Live log evidence:

```
[responses-adapter] error: ErrorResponse: Request originated from IP 135.116.244.51
...
Note: `x-ms-aad-applied-role-assignment` was present in the 403 response — identity auth worked; the block was purely network-level IP filtering.

**Secondary (misclassification bug):** `handleError()` in `responses.ts` returned 502 (not 500) for Cosmos errors because `errMsg.includes("Azure")` matched `Microsoft.Azure.Documents.Common` in the Cosmos SDK error message. The 502 was meant only for AOAI/model upstream failures; Cosmos DB errors got incorrectly bucketed into the same status code.

**Discrepancy:** The Bicep is correct — `publicNetworkAccess: publicNetworking ? 'Enabled' : 'Disabled'` with `publicNetworking=true` should keep Cosmos accessible. The `Disabled` state was introduced by a manual portal/CLI change after the last `azd provision`. The fix aligns the live state back to the Bicep intent. `azd provision` will enforce `Enabled` on future full infrastructure redeploys.

---

## Fix

### 1. Cosmos DB network access (immediate, CLI)

```bash
az cosmosdb update -n advisor-cosmos-uwmrjzgkhs2hk -g rg-advisor-dev --public-network-access Enabled
```

Confirmed: `publicNetworkAccess: Enabled` after update.

Security posture is maintained: `disableLocalAuth: true` means no connection strings or account keys are usable — all callers must authenticate via managed identity + Cosmos DB data-plane RBAC. Public network access with disabled local auth is the correct posture for ACA workloads without VNET integration.

### 2. `handleError()` misclassification fix (`agent/src/adapter/responses.ts`)

Removed `errMsg.includes("Azure")` from the `isModelError` discriminator. The check was too broad. Cosmos SDK error messages contain `Microsoft.Azure.Documents.Common/...` which satisfied the match. After fix, only genuine AOAI indicators trigger 502: `openai`, `AOAI`, `cognitiveservices`, `Service unavailable`, `timeout`, `rate limit`, HTTP 429/503.
...
|-------|-----------|-----------|
| `GET /health` | 200 | 200 |
| `GET /sessions` (no token) | 502 | 401 |
| ACA logs | `403 Cosmos firewall blocked` errors | Clean startup, no Cosmos errors |
| Tests | 34/34 | 34/34 |
| Cosmos `publicNetworkAccess` | `Disabled` | `Enabled` |

---

## Follow-ups

1. **Parker:** Add a post-provision smoke-test assertion that `az cosmosdb show ... --query publicNetworkAccess` returns `Enabled`. This would have caught the manual drift immediately.
2. **Parker:** Consider adding `preventManualBicepDrift: true` policy (Azure Policy: "Cosmos DB accounts must have public network access enabled") if the security team approves, to prevent silent manual overrides.
3. **Long-term:** VNet integration for ACA → Cosmos DB private endpoint is the hardened path. Until then, public network + RBAC + disabled local auth is appropriate.

---

#### parker-private-networking
